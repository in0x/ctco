...
u32 multiply(u32 sml, u32 lrg);

u32 recursive_multiply(u32 a, u32 b) {
    int sml = a < b ? a : b;
    int lrg = a > b ? a : b;
    return multiply(sml, lrg); 
}
...
u32 multiply(u32 sml, u32 lrg) {
    ...
    u32 half_sml = sml >> 1;
    u32 side1 = recursive_multiply(half_sml, lrg);
    u32 total = side1 + side1;
    if (sml & 1) {
        total += lrg;
    }
    return total;
